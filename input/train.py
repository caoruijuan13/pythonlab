...
default_station = [u"杭州东", u"杭州", u"上海虹桥", u"无锡", u"无锡东", u"济南西", u"北京南"]
...
def get_travel(info):
    ...
    for i in dcc:
        ...
        k += 1
    print(u"from " + start_station_name + u" to " +
          default_to_station_name + u" yes(y) or no(n)?")

    to_station_index = 0
    to_station_name = "y"  # input()
    if to_station_name == "y" and default_to_station_index > 0:
...
def get_info(line):
    ...
    if start == 0:
        return info["site_name"],"NULL",0,0,info["train_number"], info["seat_info"], info["ticket_gate"], info["month"], info["day"], info["hour"] + u":" + info["min"], str(int(info["hour"])+1) + u":" + info["min"]
...
def make_ics(line, id):
    start_station_name, to_station_name, cost_hour, cost_mins, cc, zc, djk, month, day, start_time, end_time = get_info(
        line)
    ...
    event.add('dtstart',
              datetime(int(year), int(month), int(day), int(start_hour), int(start_min), 0,
                       tzinfo=pytz.timezone("Asia/Shanghai")))
    event.add('dtend',
              datetime(int(year), int(month), int(day), int(end_time.split(":")[0]), int(end_time.split(":")[1]), 0,
                       tzinfo=pytz.timezone("Asia/Shanghai")))
    event.add('summary', start_station_name + u'-' + to_station_name + u", " + start_hour+u":"+start_min+u", "+djk)
    event.add('LOCATION', cc + u"次" + zc + u", " + str(cost_hour) + u"小时" + str(cost_mins) + u"分")
    event.add('DESCRIPTION', line)
    cal.add_component(event)

    #set this message is_read=1
    set_read_message()
